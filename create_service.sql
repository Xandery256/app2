...
BEGIN
    ...
    if (select Svc_DateTime from service where Svc_DateTime = date_time) is not null then set result = "A service already exists at that time";
        ...
        insert into service_item (Service_Item_ID, Service_ID, Seq_Num, Event_Type_ID)
			values ((select Max(Service_Item_ID)+1, Max(service_ID)+1, Seq_Num, Event_Type_ID from service_item where service_ID = service_template));
	end if;
END$$